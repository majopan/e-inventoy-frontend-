...
function FloorPlan() {
  ...
  // NUEVO: Estado para manejar archivos de importación específicos por piso
  const [importFileData, setImportFileData] = useState(null)
  const [importFileName, setImportFileName] = useState("")
  const [importTargetPiso, setImportTargetPiso] = useState("")

  // Función para cargar todos los pisos disponibles
  const fetchPisos = useCallback(
    async (sedeId = null) => {
      try {
        ...
        const filteredPisos = pisosData.filter((piso) => {
          ...
          console.log(`🔍 Comparando piso ${piso.nombre}: sede del piso=${pisoSede}, sede seleccionada=${sedeId}`)
        ...
        console.log("🎯 Pisos formateados:", formattedPisos)
        ...
        showNotification(`Error al cargar los pisos para la sede seleccionada: ${error.message}`, "error")
        setPisos([])
        setSelectedPiso("")
        setNewPosition((prev) => ({
          ...prev,
  ...
  const fetchSelectorData = useCallback(async () => {
    try {
      ...
      try {
        ...
        console.log("📱 Dispositivos cargados:", dispositivosData.length)
      } catch (error) {
      ...
      try {
        ...
        console.log("🏢 Sedes cargadas:", sedesResponse.data?.length || 0)
        console.log("🔧 Servicios cargados:", serviciosResponse.data?.length || 0)
      } catch (error) {
        console.error("❌ Error cargando servicios/sedes:", error)
        setServicios([])
        setSedes([])
      }
    } catch (error) {
  ...
  // FUNCIÓN OPTIMIZADA PARA CARGAR SOLO POSICIONES DEL PISO SELECCIONADO
  const fetchPositions = useCallback(
    async (specificPiso = null) => {
      try {
        ...
        console.log("🔄 Cargando posiciones solo para el piso:", targetPiso)
        setLoading(true)

        // FILTRAR POR PISO ESPECÍFICO EN LA URL
        const url = `${API_URL}api/posiciones/?piso=${targetPiso}`
        ...
        while (nextUrl) {
          ...
          console.log(`📋 Posiciones obtenidas para piso ${targetPiso}:`, positionsData.length)
          allPositions = allPositions.concat(positionsData)
        }

        console.log(`✅ Total de posiciones cargadas para piso ${targetPiso}:`, allPositions.length)

        // Procesamiento de posiciones específicas del piso
        const positionsObj = allPositions.reduce((acc, pos) => {
          try {
            // VERIFICAR QUE LA POSICIÓN PERTENECE AL PISO CORRECTO
            if (String(pos.piso) !== String(targetPiso)) {
              console.log(`⚠️ Posición ${pos.id} no pertenece al piso ${targetPiso}, omitiendo`)
              return acc
            }

            console.log(`🔧 Procesando posición ID: ${pos.id}, Nombre: ${pos.nombre}, Piso: ${pos.piso}`)

            // Buscar el piso correspondiente para obtener la sede
            const pisoCorrespondiente = pisos.find((p) => p.id === pos.piso || p.value === pos.piso.toString())
            const sedeDelPiso = pisoCorrespondiente ? pisoCorrespondiente.sede : null
            ...
            const normalizedPos = {
              ...
              piso: pos.piso,
              sede: sedeDelPiso || pos.sede || pos.sede_id,
              sede_nombre: pos.sede_nombre,
        ...
        console.log("📊 Posiciones procesadas para piso", targetPiso, ":", Object.keys(positionsObj).length)

        setPositions(positionsObj)
        updateAssignedDevices(positionsObj)

        console.log("✅ Posiciones del piso cargadas y estado actualizado")
      } catch (error) {
        console.error("❌ Error al cargar las posiciones del piso:", error)
        if (error.response) {
          console.error("Respuesta del servidor:", error.response.status, error.response.data)
        }
        showNotification("Error al cargar las posiciones del piso", "error")
      } finally {
        setLoading(false)
      }
    },
  ...
  useEffect(() => {
    if (sedes.length > 0 && !selectedSede) {
      console.log("🎯 Auto-seleccionando primera sede:", sedes[0].id)
      setSelectedSede(sedes[0].id.toString())
    }
  }, [sedes, selectedSede])

  useEffect(() => {
    console.log("🔄 Sede seleccionada cambió:", selectedSede)
    if (selectedSede) {
      fetchPisos(selectedSede)
    } else {
      setPisos([])
      setSelectedPiso("")
      setPositions({}) // LIMPIAR POSICIONES CUANDO NO HAY SEDE
    }
  }, [selectedSede, fetchPisos])
  ...
  useEffect(() => {
    if (selectedPiso && pisos.length > 0) {
      ...
      console.log("🔄 No hay piso seleccionado, limpiando posiciones")
      setPositions({})
      updateAssignedDevices({})
    }
  }, [selectedPiso, pisos.length])

  // Actualizar sede cuando cambie el piso seleccionado
  useEffect(() => {
    if (selectedPiso && pisos.length > 0) {
      const pisoSeleccionado = pisos.find((p) => p.value === selectedPiso)
      if (pisoSeleccionado && pisoSeleccionado.sede) {
        console.log("🔄 Actualizando sede basada en piso seleccionado:", pisoSeleccionado.sede)
        setSelectedSede(pisoSeleccionado.sede.toString())
      }
    }
  }, [selectedPiso, pisos])
  ...
  const getServiceColor = (serviceId) => {
    ...
    // Si el servicio es un objeto, obtener el color directamente
    if (typeof serviceId === "object" && serviceId.color) {
      return serviceId.color
    }

    // Si es un ID, buscar en la lista de servicios
  ...
  const getServiceName = (serviceId) => {
    ...
    // Si el servicio es un objeto, obtener el nombre directamente
  ...
  // FUNCIÓN MEJORADA DE IMPORTACIÓN - ARREGLADA PARA IMPORTAR EL ARCHIVO CORRECTO
  const importFromExcel = async (e) => {
    ...
    // LIMPIAR ESTADOS PREVIOS DE IMPORTACIÓN
    setImportFile(null)
    setImportFileData(null)
    setImportFileName("")
    setImportTargetPiso("")

    // LEER EL ARCHIVO INMEDIATAMENTE PARA EVITAR PROBLEMAS DE REFERENCIA
    ...
    reader.onload = (event) => {
      try {
        ...
        // GUARDAR LOS DATOS DEL ARCHIVO EN EL ESTADO
        ...
        setConfirmAlert({
          show: true,
          message: `¿Estás seguro de que deseas importar posiciones desde el archivo "${file.name}"?`,
          onConfirm: () => {
  ...
  const processImportFile = async () => {
    ...
    // USAR EL PISO SELECCIONADO EN EL MODAL O EL PISO ACTUALMENTE SELECCIONADO
    ...
    console.log("🎯 Piso objetivo:", targetPiso)

    showNotification(`Procesando archivo ${importFileName} para el piso ${getPisoName(targetPiso)}...`, "success")
    ...
    try {
      ...
      const confirmAction = window.confirm(
        `¿Cómo desea proceder con la importación del archivo "${importFileName}" en el piso "${getPisoName(targetPiso)}"?\n\n` +
      ...
      // OBTENER POSICIONES EXISTENTES PARA EL PISO ESPECÍFICO
      const existingPositions = Object.values(positions).filter((pos) => String(pos.piso) === String(targetPiso))
      console.log("📋 Posiciones existentes en el piso:", existingPositions.length)
      ...
      // CREAR MAPA DE POSICIONES EXISTENTES ESPECÍFICO PARA EL PISO
      const existingPositionsMap = {}
      existingPositions.forEach((pos) => {
        pos.mergedCells.forEach((cell) => {
          const key = `${cell.row}-${cell.col}-${targetPiso}`
      ...
      for (const mergeInfo of mergedCellsInfo) {
        ...
        const position = {
          ...
          piso: targetPiso, // USAR EL PISO ESPECÍFICO SELECCIONADO
        ...
        const mainCellKey = `${position.fila}-${position.columna}-${targetPiso}`
        ...
        if (confirmAction && existingPosition) {
          ...
          try {
            console.log("➕ Creando nueva posición combinada:", position.nombre, "en piso", targetPiso)
      ...
      for (let row = range.s.r; row <= range.e.r; row++) {
        for (let col = range.s.c; col <= range.e.c; col++) {
          ...
          const cellKey = `${actualRow}-${colLetter}-${targetPiso}`
          ...
          if (confirmAction && existingPosition) {
            ...
            try {
              console.log("➕ Creando nueva posición individual:", position.nombre, "en piso", targetPiso)
              const response = await axios.post(`${API_URL}api/posiciones/`, position)
      ...
      const resultMessage = `Importación de "${importFileName}" completada en piso "${getPisoName(targetPiso)}": ${savedCount} posiciones nuevas, ${updatedCount} posiciones actualizadas, ${skippedCount} celdas vacías omitidas${
  ...
  const savePositionData = async () => {
    try {
      ...
      // APLICAR COLOR DEL SERVICIO SI EXISTE
      const dataToSend = {
  ...
  // CONTROLES DE ZOOM MEJORADOS - PERMITIR ZOOM MÁS PEQUEÑO
  const handleZoomIn = () => setZoomLevel((prev) => Math.min(prev + 0.1, 3)) // Aumenté el máximo a 3x
  const handleZoomOut = () => setZoomLevel((prev) => Math.max(prev - 0.1, 0.1)) // Reduje el mínimo a 0.1 (10%)
  ...
  const getServiceStatistics = () => {
    const positionsToAnalyze = showAllPositions
      ? Object.values(positions).filter((pos) => {
          const pisoSeleccionado = pisos.find((p) => p.value === selectedPiso)
          return pisoSeleccionado ? pos.sede == pisoSeleccionado.sede : false
        })
      : Object.values(positions).filter((pos) => pos.piso === selectedPiso)
      ...
      const normalizedPosition = {
        ...
        dispositivos: Array.isArray(positionCopy.dispositivos)
          ...
          : positionCopy.dispositivos
            ...
            : [],
        servicio: (() => {
          // Si el servicio es un objeto, obtener su ID
    ...
    return (
      <td
        ...
        style={{
          ...
          // Enhanced visual indicator for positions
  ...
  // FUNCIÓN PARA MANEJAR EL CAMBIO DE PISO Y CARGAR SOLO SUS POSICIONES
  const handlePisoChange = (pisoValue) => {
    console.log("🔄 Cambiando a piso:", pisoValue)
    setSelectedPiso(pisoValue)
    // Las posiciones se cargarán automáticamente por el useEffect
  }
  ...
  return (
    <div className="dashboard-container">
      <div className="controls-container">
        <div className="tabs">
          {pisos.length > 0 ? (
            ...
            <div className="no-floors-message">No hay pisos disponibles</div>
          )}
        </div>
              ...
              <tbody>
                {Array.from({ length: rows }, (_, i) => i + 1).map((row) => (
                  <tr key={row}>
                    ...
                    {columns.map((col) => {
                      // Búsqueda de posiciones optimizada
      ...
      {/* MODAL MEJORADO PARA SELECCIÓN DE PISO EN IMPORTACIÓN */}
      {isImportModalOpen && (
        <div className="modal-overlay">
              ...
              {(importTargetPiso || selectedPiso) && (
                ...
                >
                  <p style={{ margin: "0", color: "#155724" }}>
                    ...
                    <strong>"{getPisoName(importTargetPiso || selectedPiso)}"</strong>
                  </p>
                </div>
              )}
            </div>
