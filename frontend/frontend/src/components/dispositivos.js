...
const Dispositivos = () => {
  ...
  const fetchPosiciones = async () => {
    try {
      ...
      setPosiciones(Array.isArray(data) ? data : [])
    } catch (error) {
  ...
  const fetchSedes = async () => {
    try {
      ...
      const sedesFormateadas = data.map(sede => ({
        id: sede.id || sede.pk || sede.ID || 0,
        nombre: sede.nombre || sede.name || sede.Nombre || `Sede ${sede.id || sede.pk || ''}`
      }))
  ...
  const addDevice = async () => {
    try {
      const deviceToSend = {
        ...
        sede: newDevice.sede ? Number(newDevice.sede) : null
      }
  ...
  const updateDevice = async () => {
    try {
      const cleanDeviceData = {
        ...
        sede: selectedDevice.sede ? Number(selectedDevice.sede) : null
      }
...
const DeviceForm = ({ device, setDevice, onSubmit, posiciones, sedes, choices }) => {
